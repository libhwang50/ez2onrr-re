...
| `server/re/_capture_addon.py` | **the mitmproxy capture addon (sweep/RE only)** — wraps the offline core: forwards `login`/`pattern`/uncached CDN upstream and files the returning CDN bodies into `extracted_charts/`. No longer the server |
...
| `server/re/_sweep.py` | **drive the game to capture charts** (a capture counts only once the CDN body arrives; captures are filed into `extracted_charts/` by `_capture_addon.py`) — blind (the server log is the sensor), guarded by a focused-window check, with `--probe`-style `--calibrate` that deduces the difficulty/keymode keys from the request JSON |
