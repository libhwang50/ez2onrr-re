...
const TARGET={zf:1,da:1,vx:1,wa:1,wb:1,wy:1,qe:1};
  ...
  for(let i=0;i<n;i++){
    ...
    for(let c=0;c<cnt;c++){
      ...
      while(true){
        ...
        let val=null;
        if(isStatic && ftype==="System.String"){
          const vp=Memory.alloc(Process.pointerSize);
          try{FSV(fld,vp); val=RS(vp.readPointer());}catch(e){val="<err>"}
        }
        out.push({cls:cn,field:fname,type:ftype,static:isStatic,val:val});
      }
    }
  }
  return out;
